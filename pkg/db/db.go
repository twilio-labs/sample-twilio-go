...
func (db *DB) GetCustomerByID(ctx context.Context, id string) (*configuration.Customer, error) {
	...
	err := db.QueryRowContext(ctx, "SELECT * FROM customers WHERE id = $1", id).Scan(&customer.ID, &customer.Name, &customer.Email, &customer.CreatedAt)
	if err != nil {
		return nil, err
	}
	return customer, nil
...
func (db *DB) GetCustomerByEmail(ctx context.Context, email string) (*configuration.Customer, error) {
	customer := &configuration.Customer{}
	err := db.QueryRowContext(ctx, "SELECT * FROM customers WHERE email = $1", email).Scan(&customer.ID, &customer.Name, &customer.Email, &customer.CreatedAt)
	if err != nil {
		return nil, err
	}
	return customer, nil
...
func (db *DB) CreateCustomer(ctx context.Context, customer *configuration.Customer) error {
	...
	_, err = db.ExecContext(ctx, "INSERT INTO customers (name, email, phone_number) VALUES ($1, $2, $3)", customer.Name, customer.Email, customer.PhoneNumber)
	if err != nil {
		return err
	}
	return nil
...
func (db *DB) GetCustomerByPhoneNumber(ctx context.Context, phoneNumber string) (*configuration.Customer, error) {
	customer := &configuration.Customer{}
	err := db.QueryRowContext(ctx, "SELECT * FROM customers WHERE phone_number = $1", phoneNumber).Scan(&customer.ID, &customer.Name, &customer.Email, &customer.PhoneNumber, &customer.CreatedAt)
	if err != nil {
		return nil, err
	}
	return customer, nil
